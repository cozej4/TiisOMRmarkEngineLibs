...
>
  ...
  <xsl:template match="/csd:CSD">
    ...
    <xsl:apply-templates select="csd:facilityDirectory"/>
    <xsl:apply-templates select="csd:providerDirectory"/>

  </xsl:template>
  <xsl:template match="csd:organizationDirectory">
    <xsl:apply-templates select="csd:organization"/>  
  </xsl:template>
  <xsl:template match="csd:facilityDirectory">
    <xsl:apply-templates select="csd:facility"/>
  </xsl:template>

  <xsl:template match="csd:providerDirectory">
    <xsl:apply-templates select="csd:provider"/>
  </xsl:template>
  <xsl:template match="csd:organization">
    <xsl:if test="idea:startsWith('TZ.NT.AS', csd:otherID/@code) or idea:startsWith('urn:uuid:EDFD5B83-560D-39EB-9121-D8EB33326F1D', csd:parent/@entityID) or
            idea:startsWith('urn:uuid:EDFD5B83-560D-39EB-9121-D8EB33326F1D', @entityID)">
      ...
      <xsl:choose>
        <xsl:when test="csd:parent">
          ...
          '<xsl:value-of select="idea:safe(@entityID)"/>'
          FROM "PLACE" WHERE "CODE" = '<xsl:value-of select="idea:safe(../csd:organization[@entityID = $parentId]/@entityID)"/>';
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="csd:facility">
      INSERT INTO "HEALTH_FACILITY" ("ID", "NAME", "CODE", "PARENT_ID", "LEAF", "MODIFIED_ON", "MODIFIED_BY", "VACCINATION_POINT", "VACCINE_STORE", "TYPE_ID")
      VALUES (nextval('"HEALTH_FACILITY_ID_seq"'), '<xsl:value-of select="idea:safe(csd:primaryName/text())"/>', '<xsl:value-of select="idea:safe(@entityID)"/>', 
      COALESCE((SELECT "ID" FROM "HEALTH_FACILITY" WHERE "CODE" = '<xsl:value-of select="./csd:organizations/csd:organization/@entityID"/>'),  true, CURRENT_TIMESTAMP, 1, <xsl:value-of select="csd:codedType/@code = 4"/>, true,
      (SELECT "ID" FROM "HEALTH_FACILITY_TYPE" WHERE "CODE" = '<xsl:value-of select="csd:codedType/@code"/>'));
  </xsl:template>


  <xsl:template match="csd:provider">
    INSERT INTO "USER" ("ID", "USERNAME", "PASSWORD", "FIRSTNAME", "LASTNAME", "IS_ACTIVE", "HEALTH_FACILITY_ID", "LASTLOGIN")
    SELECT NEXTVAL('"USER_ID_seq"'), '<xsl:value-of select="idea:safe(concat(./csd:demographic/csd:name/csd:surname,./csd:demographic/csd:name/csd:forename))"/>', 'aIbv4TPRfPLM/N8X8N+KECDJrwE=',
      '<xsl:value-of select="idea:safe(./csd:demographic/csd:name/csd:forename)"/>', '<xsl:value-of select="idea:safe(./csd:demographic/csd:name/csd:surname)"/>', true, "ID", CURRENT_TIMESTAMP FROM
      "HEALTH_FACILITY" WHERE "CODE" = '<xsl:value-of select="./csd:organizations/csd:organization/@entityID"/>';
  </xsl:template>
